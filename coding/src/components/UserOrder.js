...
const UserOrder = () => {
    const [userOrderDeatils, setUserOrderDetails] = useState("")
    const onLoad = (event) => {
        fetch(bakendBaseUrl + "/orders/getAllOrders", {
            method: 'GET',
            headers: bakendHeader,
        }).then((response) => response.json())
            .then((json) => { setUserOrderDetails(json) });
    }
    ...
    return (
        <div>
            {userOrderDeatils && userOrderDeatils.map((order) => (
                <div className="user-order">
                    <div className="order-details">
                        <div className="orderid">
                            <label htmlFor="1">
